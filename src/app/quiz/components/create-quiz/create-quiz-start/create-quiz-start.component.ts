...
@Component({
  selector: 'app-create-quiz-start',
  templateUrl: './create-quiz-start.component.html',
  providers: [NgbProgressbarConfig],
  styleUrls: ['./create-quiz-start.component.scss'],
  animations: [
    trigger('stageEnter', [
      transition(':enter', [
        style({ opacity: 0 }),
...
export class CreateQuizStartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.form.controls['is_public'].valueChanges.subscribe(() => {
      this.initUpdateValidatorsFormControl();
    });
  }
  ...
  initUpdateValidatorsFormControl(){
    if (this.form.controls.is_public.value === 'true') {
      this.updateValidatorsFormControl(this.f['recipients'], null);
    } else {
      this.updateValidatorsFormControl(
        this.f['recipients'],
        Validators.compose([
