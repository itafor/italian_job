...
export class PaymentComponent implements OnInit {
  ...
  limit = 10;
  currencies = [];
  @ViewChild('paymentTabSet') refNgbTabset: NgbTabset;
  openTabs: any[] = [];
  ...
  paymentSuccessHandler(paymentCreated, tabIndex: number): void {
    this.closeTab(null, tabIndex);
    this.getVendors();
  }

  viewPaymentHandler(vendor): void {
  ...
  addToTabs(title?: string, tabContent?: any, type?: string): void {
    let newTab;
    if (!title) {
      newTab = { title: 'New Payment', type: 'NEW_PAYMENT' };
    } else if (type === 'VIEW_PAYMENT') {
      newTab = { title, content: tabContent, type: 'VIEW_PAYMENT' };
    } else {
      newTab = { title, content: tabContent, type: 'UPDATE_PAYMENT' };
    }
  ...
  getVendor(content,dataRow) {
    this.addToTabs('New Payment', content, 'UPDATE_VENDOR');
    this.focusOnCreatedTab();
  ...
  storeExpense(token = null) {
   ...
   const reason = this.f.reason.value;
    this.service.addExpense(this.paymentTypeOption, amountPayable,reason,category, bankAccount,
      amountPaidViaBank, creditAmount, cashPaid, vendor, token)
      .subscribe(
        data => {
