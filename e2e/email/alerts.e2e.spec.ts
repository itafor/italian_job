...
describe('Quabbly Email submodule tests | alerts Tab', () => {
  const page: Email = new Email(MailListEnums.ALERTS);
  const pagMeta: {} = {};
  const currentlyViewing = {};
  let hasAccessToAlerts: boolean;
  const thisBoxType = 'alerts';
  ...
  it('should tab over to alerts', () => {
    page.getTab(2).then(alertsTab => {
      alertsTab.click().then(() => {
        alertsTab.$('a').getAttribute('class').then(className => {
  ...
  it('should actually change displayed tab to alerts tab', () => {
    page.getTabContent().then(content => {
      content.getAttribute('id').then(id => {
        expect(id).toBe(Email.defaultTabs[2].contentId);
      });
    });
  });
  ...
  it('should exhibit expected pagination behaviour', () => {
    page.checkIfIGotAnError(MailListEnums.ALERTS).then(errorState => {
      hasAccessToAlerts = !errorState;
      if (hasAccessToAlerts) {
        // proceed with tests - else no point
        // change account
        utils.getPaginationIndicatorsFor(MailListEnums.ALERTS).then(indicators => {
